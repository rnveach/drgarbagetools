...
/**
 *  Figure for the invoke instruction.
 *
 * @author Sergej Alekseev  
 *  @version $Revision:143 $
 *  $Id:InvokeVertexFigure.java 143 2007-05-28 10:10:03Z aleks $
 */
public class InvokeVertexFigure extends Shape {
	 ...
	 */
	public InvokeVertexFigure(boolean useGradientFillColor) {
		super();
...
//	    graphics.drawPolygon(getPoints(r));
	...
	private static PointList getPoints(Rectangle r){
	    ...
	    pl.addPoint(r.x + r.width/2 + r.width/4, r.y + r.height - 1);
	...
	private static PointList getPointsOutside(Rectangle r){
	    ...
	    pl.addPoint(r.x + r.width/2 + r.width/4, r.y);
	    pl.addPoint(r.x + r.width - 1, r.y + r.height/2);
	    pl.addPoint(r.x + r.width/2 + r.width/4 - 2, r.y + r.height + 1);
	    pl.addPoint(r.x + r.width/2 + r.width/4 - 2, r.y + r.height + 2);
	    pl.addPoint(r.x + r.width + 1, r.y + r.height + 2);
	    pl.addPoint(r.x + r.width + 1, r.y + r.height + 1);
	    return pl;
	}
}
